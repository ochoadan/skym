...
class ServiceClient:
    ...
    def _http(self, method, path, body=None, authenticated=True):
        ...
        try:
            ...
            if response.status != 200:
                ...
                if (type(value["protocolVersion"]) is not int or value["protocolVersion"] != 1 or
                ...
                known = {"invalid_credentials", "invalid_session", "expired_session", "wrong_community",
                         ...
                         "interaction_capacity", "session_capacity", "request_id_conflict", "rate_limited"}
                code = value["error"]
    ...
    @staticmethod
    def _state(value):
        fields(value, "interactionId revision status message")
        guid(value["interactionId"])
        if (type(value["revision"]) is not int or not 0 <= value["revision"] <= 256 or
    ...
    def connect(self):
        ...
        value = self._http("POST", "/sessions", {
            "protocolVersion": 1, "communityId": self.config.community, "requestId": request_id,
    ...
    def probe(self, request_id, cancelled=lambda: False):
        try:
            ...
            return value["state"]["message"], value["state"]["revision"]
        except (ValueError, TypeError, KeyError, RecursionError):
            raise ServiceError("invalid_response") from None
...
def main():
    ...
    allowed = Path(__file__).resolve().parents[2] / "local" / "t04-3"
    if not destination.is_relative_to(allowed) or destination == allowed:
        raise ValueError("Output must be a new file under local/t04-3")
